...
Kotlin + Jetpack Compose is the primary implementation for new Offzone work. iOS remains a product and visual parity reference. Android 8.0+ (API 26), target 36. Open this folder in Android Studio.
...
Implemented: goal/window onboarding with preset, custom, and goal-suggested hours; staged app/place/schedule/review rule setup; ready screen and dismissible Pro preview after successful activation; saved/applied place rules; app selection; daily and overnight schedules; precise arrival confirmation and explicit focus; optional user-started place monitoring; AccessibilityService shield and free recovery; quick timed focus; Firebase email/Google/Apple account flows; verified goal sync; RevenueCat identity/expiry/purchase/restore flows; private plans/reflections/weekly summaries; JSON import/export/share; English/Korean SUIT UI and **static Nook Cat** (8 expressions + 4 full-body resources). Android 13+ lists English and Korean in system per-app language settings. The home card uses warm ivory when scheduled or paused and charcoal only during focus. Color-fringe/rights/physical-device QA remains before release.
...
- Nook uses static transparent artwork; there is no animated mascot runtime.
