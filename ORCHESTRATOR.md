...
- **제품 범위:** Android를 앞으로의 주 개발 기준으로 삼고 iOS와 핵심 화면·흐름을 맞춘 디지털 자기관리 앱. 각 운영체제의 차단 권한과 앱 선택 방식은 플랫폼 고유 제약을 따른다. 운동·식단·의료 등 모든 자기관리 기능을 추가한다는 뜻은 아니다.
...
| D-45 | 사용자 첫 설정 시간·Pro 화면 순서 수정·2026-09-25 | ACT-01: iOS 시간 선택에 기존 아침/오후/저녁 외 **Another time**(기기 시간 선택기, 선택 시 1시간 구간)과 **Not sure yet**(선택 목표의 추천 1시간 구간)을 추가한다. RULE-01/OFFER-01: 첫 규칙 저장 뒤 `Activate my rule`이 성공하면 홈 위에 닫을 수 있는 `RoomPaywallView`를 바로 표시한다. 활성화 실패에는 표시하지 않으며 `Go to home`과 `Continue with free`는 구매 없이 진행한다. `Restore access`는 계속 무료다. 첨부 영상은 화면 참고이며 7일 체험·표시 가격을 실제 상품 약속으로 복제하지 않는다. `RoomProOfferReady=false` 및 실구매·실기기 검증 게이트 유지. 현재 iOS 코드 경로에 반영했고 Android 동등 흐름은 후속 확인 대상이다. |
| D-46 | Android 우선 개발·iOS 화면 흐름 동기화·2026-09-25 | 사용자가 앞으로 Android를 주 개발 기준으로 지정했다. Android에 Nook과 버터/아이보리/차콜 홈 상태 카드, 직접 시간·목표별 추천 선택, 단계별 규칙 작성, 저장 후 Ready, 성공한 활성화 뒤 닫을 수 있는 Pro 화면을 연결한다. 앱별 영어·한국어 언어 설정을 노출한다. 판매 준비 플래그는 꺼진 상태로 실제 가격·체험을 주장하지 않으며 명시적 집중 시작과 무료 접근 복구를 유지한다. Screen Time과 Android Accessibility, Apple 앱/웹 토큰과 Android 패키지 선택은 운영체제 차이로 동일 구현이 될 수 없다. 에뮬레이터/빌드와 서명 AAB 증거는 실행 기록에 따르며 Play 게시·실기기 권한/구매 검증과 구분한다. |
