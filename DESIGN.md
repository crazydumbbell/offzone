...
계정의 Nook 카드에는 홈의 설정 필요 카드와 같은 #EFE5AB을 써서 거의 흰색으로 보이던 배경을 맞춘다.

## 홈 상태 카드 색상 — 2026-09-25

iOS 홈의 예약·일시중지 상태는 버터색 배경에 맞는 따뜻한 아이보리 카드를 사용한다. 차콜 카드는 실제 집중 중에만 사용하며, 안전 복구 상태의 민트는 유지한다. 일정 시작 전에는 실행할 수 없는 `Start focus` 버튼을 표시하지 않고 시작 시각·위치 안내를 보여준다.
